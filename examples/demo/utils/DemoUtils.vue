...
const items: Record<string, UtilItem> = {
  rgbaToHex: {
    key: "rgbaToHex",
    name: "rgbaToHex",
...
rgbaToHex("rgb(0, 128, 255)");     // "#0080ff"`,
    api: [
      { name: "color", type: "string", default: "—", required: true, desc: "rgba 或 rgb 颜色字符串" },
      { name: "返回值", type: "string", default: "—", required: true, desc: "十六进制颜色字符串，如 #ff0000" },
    ],
  },
  hexToRGBA: {
    key: "hexToRGBA",
...
hexToRGBA("#ff0000");       // "rgb(255, 0, 0)"`,
    api: [
      { name: "hex", type: "string", default: "—", required: true, desc: "十六进制颜色字符串 #RRGGBB" },
      { name: "alpha", type: "number", default: "—", required: false, desc: "可选，透明度 0~1" },
      { name: "返回值", type: "string", default: "—", required: true, desc: "rgba 或 rgb 字符串" },
    ],
  },
  colorToRGBA: {
    key: "colorToRGBA",
...
colorToRGBA("#ff5733");  // { r: 255, g: 87, b: 51 }`,
    api: [
      { name: "hex", type: "string", default: "—", required: true, desc: "十六进制颜色字符串" },
      { name: "返回值", type: "{ r: number; g: number; b: number }", default: "—", required: true, desc: "RGB 对象" },
    ],
  },
  getRandom: {
    key: "getRandom",
...
getRandom(0, 100);  // 0~100 随机整数`,
    api: [
      { name: "min", type: "number", default: "—", required: true, desc: "最小值" },
      { name: "max", type: "number", default: "—", required: true, desc: "最大值" },
      { name: "返回值", type: "number", default: "—", required: true, desc: "范围内随机整数" },
    ],
  },
  scaleFormat: {
    key: "scaleFormat",
...
scaleFormat("3.14159", 3);            // "3.142"`,
    api: [
      { name: "value", type: "string", default: '"0"', required: false, desc: "待格式化数据" },
      { name: "scale", type: "number", default: "2", required: false, desc: "保留小数位数" },
      { name: "返回值", type: "string", default: "—", required: true, desc: "格式化后的字符串" },
    ],
  },
  flattenObj: {
    key: "flattenObj",
...
// { "prefix.a.b": 1 }`,
    api: [
      { name: "obj", type: "Record<string, any>", default: "—", required: true, desc: "需要扁平化的对象" },
      { name: "prefix", type: "string", default: '""', required: false, desc: "键名前缀" },
      { name: "返回值", type: "Record<string, any>", default: "—", required: true, desc: "扁平化后的单层对象" },
    ],
  },
  unFlatten: {
    key: "unFlatten",
...
// { a: { b: 1 }, c: 2 }`,
    api: [
      { name: "obj", type: "Record<string, any>", default: "—", required: true, desc: "扁平化的对象" },
      { name: "返回值", type: "Record<string, any>", default: "—", required: true, desc: "还原后的嵌套对象" },
    ],
  },
  isObjEqual: {
    key: "isObjEqual",
...
isObjEqual({ a: 1 }, { a: 2 });                              // false`,
    api: [
      { name: "obj1", type: "Record<string, any>", default: "—", required: true, desc: "对象一" },
      { name: "obj2", type: "Record<string, any>", default: "—", required: true, desc: "对象二" },
      { name: "返回值", type: "boolean", default: "—", required: true, desc: "是否相同" },
    ],
  },
  isObjEmpty: {
    key: "isObjEmpty",
...
isObjEmpty({ a: 1 }); // false`,
    api: [
      { name: "obj", type: "Record<string, any>", default: "—", required: true, desc: "需要判断的对象" },
      { name: "返回值", type: "boolean", default: "—", required: true, desc: "是否为空" },
    ],
  },
  flattenTree: {
    key: "flattenTree",
...
// [{ name: "A", children: [...] }, { name: "B" }, { name: "C" }, { name: "D", children: [] }]`,
    api: [
      { name: "arr", type: "any[]", default: "—", required: true, desc: "树形数组" },
      { name: "childrenKey", type: "string", default: '"children"', required: false, desc: "子节点字段名" },
      { name: "返回值", type: "any[]", default: "—", required: true, desc: "扁平化后的数组" },
    ],
  },
  isArrEqual: {
    key: "isArrEqual",
...
isArrEqual([1, 2], [1, 3]);        // false`,
    api: [
      { name: "arr1", type: "Array<string | number>", default: "—", required: true, desc: "数组一" },
      { name: "arr2", type: "Array<string | number>", default: "—", required: true, desc: "数组二" },
      { name: "返回值", type: "boolean", default: "—", required: true, desc: "是否相同" },
    ],
  },
  copyToClipboard: {
...
if (success) console.log("复制成功");`,
    api: [
      { name: "text", type: "string", default: "—", required: true, desc: "要复制的文本" },
      { name: "返回值", type: "Promise<boolean>", default: "—", required: true, desc: "是否成功" },
    ],
  },
  debounce: {
    key: "debounce",
    name: "debounce",
    desc: "防抖函数，延迟执行直到停止触发",
    signature: `declare const debounce: (
  fn: (...args: any[]) => any,
  delay?: number,
  immediate?: boolean
) => (...args: any[]) => void;`,
...
// 第三个参数 true 表示首次立即执行
const handleClick = debounce(fn, 500, true);`,
    api: [
      { name: "fn", type: "(...args: any[]) => any", default: "—", required: true, desc: "需要防抖的函数" },
      { name: "delay", type: "number", default: "300", required: false, desc: "延迟时间（ms）" },
      { name: "immediate", type: "boolean", default: "false", required: false, desc: "是否首次立即执行" },
      { name: "返回值", type: "(...args: any[]) => void", default: "—", required: true, desc: "防抖后的函数" },
    ],
  },
  useDebounceRef: {
    key: "useDebounceRef",
...
// keyword.value = "abc" → 500ms 后才触发依赖更新`,
    api: [
      { name: "value", type: "T", default: "—", required: true, desc: "初始值" },
      { name: "delay", type: "number", default: "300", required: false, desc: "延迟时间（ms）" },
      { name: "返回值", type: "Ref<T>", default: "—", required: true, desc: "防抖 ref" },
    ],
  },
  performChunk: {
    key: "performChunk",
    name: "performChunk",
    desc: "分片执行大任务，避免阻塞主线程导致页面卡顿",
    signature: `declare const performChunk: (
  data: number | any[],
...
performChunk(100000, (item, i) => {
  ...
  requestAnimationFrame(() => {
    ...
    api: [
      { name: "data", type: "number | any[]", default: "—", required: true, desc: "任务数据（数字=执行次数，数组=遍历）" },
      { name: "taskHandler", type: "(item, index) => void", default: "—", required: true, desc: "每项任务的处理函数" },
      { name: "scheduler", type: "(run) => void", default: "—", required: true, desc: "调度器，控制执行时机" },
    ],
  },
  concurRequest: {
    key: "concurRequest",
...
const results = await concurRequest([
  ...
  "/api/data4",
    ...
    api: [
      { name: "urls", type: "string[]", default: "[]", required: false, desc: "请求 URL 列表" },
      { name: "maxNum", type: "number", default: "3", required: false, desc: "最大并发数" },
      { name: "返回值", type: "Promise<Response[]>", default: "—", required: true, desc: "所有响应结果" },
    ],
  },
  usePermission: {
    key: "usePermission",
    name: "usePermission",
    desc: "权限管理 hook，基于位运算实现高性能权限判断",
    signature: `declare const usePermission: <T extends readonly string[]>(permissions: T) => {
...
hasPermission(v, "add");                 // false`,
    api: [
      { name: "permissions", type: "readonly string[]", default: "—", required: true, desc: "权限字段列表" },
      { name: "返回值.getPermission", type: "(p: string[]) => number", default: "—", required: true, desc: "根据字段列表获取权限值" },
      { name: "返回值.hasPermission", type: "(v, p) => boolean", default: "—", required: true, desc: "判断是否拥有某权限" },
      { name: "返回值.switchPermission", type: "(v, p) => number", default: "—", required: true, desc: "切换某权限（异或）" },
    ],
  },
  getValue: {
    key: "getValue",
    name: "getValue<T>",
    desc: "TypeScript 类型体操：将对象属性转为 getter 方法类型",
...
// { getAge: number; getName: string; }`,
    api: [
      { name: "T", type: "泛型参数", default: "—", required: true, desc: "源对象类型" },
      { name: "结果", type: "映射类型", default: "—", required: true, desc: "属性名加 get 前缀并首字母大写" },
    ],
  },
  setValue: {
    key: "setValue",
    name: "setValue<T>",
    desc: "TypeScript 类型体操：将对象属性转为 setter 方法类型",
...
// { setAge: number; setName: string; }`,
    api: [
      { name: "T", type: "泛型参数", default: "—", required: true, desc: "源对象类型" },
      { name: "结果", type: "映射类型", default: "—", required: true, desc: "属性名加 set 前缀并首字母大写" },
    ],
  },
  getOptional: {
    key: "getOptional",
    name: "getOptional<T>",
    desc: "TypeScript 类型体操：提取对象中所有可选属性",
...
// { port?: number; debug?: boolean; }`,
    api: [
      { name: "T", type: "泛型参数", default: "—", required: true, desc: "源对象类型" },
      { name: "结果", type: "条件映射类型", default: "—", required: true, desc: "仅包含可选属性" },
    ],
  },
  setOptional: {
    key: "setOptional",
...
// { name: string; age?: number; email?: string; }`,
    api: [
      { name: "T", type: "泛型参数", default: "—", required: true, desc: "源对象类型" },
      { name: "K", type: "keyof T", default: "—", required: true, desc: "要设为可选的属性联合" },
    ],
  },
  arrayToUnion: {
    key: "arrayToUnion",
...
// "add" | "edit" | "view"`,
    api: [
      { name: "T", type: "readonly any[]", default: "—", required: true, desc: "as const 数组类型" },
      { name: "结果", type: "联合类型", default: "—", required: true, desc: "数组元素的联合" },
    ],
  },
  getCompType: {
    key: "getCompType",
...
await formRef.value?.validate();`,
    api: [
      { name: "_comp", type: "组件构造函数", default: "—", required: true, desc: "Vue 组件" },
      { name: "返回值", type: "Ref<InstanceType<T>>", default: "—", required: true, desc: "组件实例 ref" },
    ],
  },
};
...
const currentItem = computed(() => items[props.activeKey] || items.rgbaToHex);
const highlightedCode = computed(() => highlightTsCode(currentItem.value.usage));
const highlightedSignature = computed(() =>
  currentItem.value.signature ? highlightTsCode(currentItem.value.signature) : "",
);
const highlightedImpl = computed(() =>
  currentItem.value.implementation ? highlightTsCode(currentItem.value.implementation) : "",
);
</script>
